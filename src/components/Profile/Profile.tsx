...
function Profile(Props: FullRouteProps) {
  ...
  const handleSubmit = (event: any) => {
    ...
    register(data).then(
      ...
      (error) => {
        if (error.statusCode === 409) alert("Email already has an account");
      }
    );
  };
  ...
  const changeInputHandler = (event: any) => {
    ...
    switch (id) {
      case "formUsername":
        ...
        if (value.length < 6)
          setValidated({
            ...validated,
            usernameValid: false,
            usernameError: "Must have 6 characters",
          });
        //Props
        else setValidated({ ...validated, usernameValid: true });
      ...
      case "formPasswordId":
        ...
        if (value.length < 4 || value.length > 10)
          ...
          setValidated({
            ...
            passwordValid: false,
            passwordError:
              "Must include at least one upper" +
              " case letter,one lower case letter, and one digit.",
          });
        else setValidated({ ...validated, passwordValid: true });
        break;
      case "formConfirmPassword":
        if (value !== profile.password)
          setValidated({
            ...
            cPasswordError: "Passwords dont match!",
          });
        //Props
        else setValidated({ ...validated, cPasswordValid: true });
        break;
      ...
      case "formPhoneNumber":
        ...
        //Props
        else setValidated({ ...validated, phoneNumberValid: true });
        break;
      case "formFirstName":
        if (value.length === 0)
          setValidated({
        ...
        //Props
        else setValidated({ ...validated, firstNameValid: true });
        break;
      case "formLastName":
        if (value.length === 0)
          setValidated({
        ...
        //Props
        else setValidated({ ...validated, lastNameValid: true });
        break;
      case "formEmail":
        if (value.length === 0)
          setValidated({
        ...
        //Props
        else setValidated({ ...validated, emailValid: true });
        break;
    }
  };
